...
- **Micro-Previews (Early Decision Gates)**: Micro-Previews trigger before expensive or irreversible actions. They cover two dimensions:
  ...
  - **User accessibility**: Before executing any task where a more interactive alternative exists (e.g., running `verify_accumulate.py` batch when `app.py` UI could provide live observation), the agent MUST pause and present a Micro-Preview naming both channels and their tradeoffs. Example: "I can start the app UI (`app.py --workspace ...`) so you can observe verification interactively, or run the batch tool (`verify_accumulate.py`) for speed. Which do you prefer?" This applies to any transition where the user's ability to observe and intervene would be reduced.
