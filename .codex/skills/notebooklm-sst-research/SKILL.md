...
- In the short report to the user, you MUST explicitly explain the semantics of the files you generated (e.g. "I created timestamped raw artifacts in `data_out/` to preserve history, and updated the `candidate.csv` pointer which serves as the 'draft' for the app's intake and verification step"). Also report: candidate rows, novelty vs default, union coverage, domains, % full-URL, % HTML. If time expires below 100, mark the run incomplete rather than calling it finished. The **Report** step MUST explicitly audit:
  - **Process quality**: timeouts, looping compliance, and user feedback raised during the run.
  - **Observability audit**: At each phase during this run, did the agent offer the user the most interactive channel available? When handing off to the intake workflow, explicitly recommend starting with `app.py --workspace` for user observability. If the agent chose batch tools at any point where an interactive alternative existed, report this as an observability gap and explain the reasoning. This prevents the next workflow phase from silently bypassing the user's ability to observe and intervene.
